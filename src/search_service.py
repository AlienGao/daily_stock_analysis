...
class SearchService:
    ...
    def search_stock_news(
        ...
        if cached is not None:
            if waited:
                logger.info("使用并发填充后的缓存搜索结果: %s(%s)", stock_name, stock_code)
            else:
                logger.info("使用缓存搜索结果: %s(%s)", stock_name, stock_code)
    ...
    def search_comprehensive_intel(
        ...
        for dim in search_dimensions:
            ...
            # 对结果进行排名
            if dim_response and dim_response.results:
                dim_response = self._rank_news_response(
                    dim_response,
                    stock_code=stock_code,
                    stock_name=stock_name,
                    prefer_chinese=self._should_prefer_chinese_news(stock_code, stock_name),
                    max_results=target_per_dimension,
                    log_scope=f"{stock_code}:{used_provider_name or 'None'}:{dim['name']}:rank",
                )
