...
class StockAnalysisPipeline:
    ...
    def analyze_stock(
        ...
        *,
        ...
        """
        # lazy import 避免子进程/多线程中模块级 import 未就绪
        ...
        try:
            ...
            # (attribute may be absent on partially-constructed instances)
            factor_signals = getattr(self, "_factor_signals_cache", {}).get(code)

            # Step 5: 获取分析上下文（技术面数据）— 提前，在 agent/non-agent 分支前执行
            _daily_code = normalize_stock_code(code)
            context = self._get_analysis_context_with_market_fallback(
                _daily_code, analysis_target=analysis_target
            )

            if context is not None:
                _today_close = context.get("today", {}).get("close")
                logger.info("[DBG] %s(%s) get_analysis_context: data_missing=%s today_close=%s", stock_name, code, context.get("data_missing"), _today_close)
            if context is None:
            ...
            # Step 5: 分析上下文（技术面数据）已在 agent/non-agent 分支前提前获取
...
# Step 6: 增强上下文数据
            ...
            if result and result.success:
                try:
                    ...
                    saved_history_id = self._save_analysis_history_row(
                        result=result,
                        query_id=query_id,
        ...
        try:
            ...
            (
                ...
                self._build_agent_analysis_artifacts(
                    code=code,
                    stock_name=stock_name,
                    base_context=base_context if base_context is not None else analysis_context,
                    market=market,
