...
class SystemConfigService:
    ...
    def _get_stocks_by_categories(self, categories: List[str]) -> str:
        """Get merged stock list from multiple categories, removing duplicates."""
        category_stocks = {
        ...
        for cat in categories:
            if cat in category_stocks:
                stocks = category_stocks[cat].split(',')
                all_stocks.update(stocks)
        
        return ','.join(sorted(all_stocks))
    ...
    def update(
        ...
        for item in items:
            ...
            if key == "STOCK_CATEGORY":
                stock_categories = [c.strip() for c in value.split(',') if c.strip()]

        # If STOCK_CATEGORY is updated, update STOCK_LIST accordingly
        if stock_categories:
            stock_list = self._get_stocks_by_categories(stock_categories)
            updates.append(("STOCK_LIST", stock_list))
            submitted_keys.add("STOCK_LIST")
