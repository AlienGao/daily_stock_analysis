...
- [新功能] 港股页右侧「日内数据」Tab 前新增「下午上涨」Tab：统计当日港股通成份股下午分钟价首次超过上午收盘价（13:00 前最后一根分钟收盘价）的个股，每只只记录首次突破的时间点与突破时相对上午收盘价的涨幅，按首次突破时间排序（最近突破在前），随 realtime 快照每分钟刷新，并展示突破/覆盖统计
...
- [修复] 券商金股回测缓存检测到当月新增股票/券商时改为全量重算并整体持久化，同时校验券商聚合完整性（旧版本只增量补 `stock_returns`，导致新增券商无胜率/累计收益、新旧结果混存）
- [修复] 券商金股月度数据在当月滚动发布期间不再因已有数据而跳过刷新：盘后任务每日覆盖落库当月金股，避免月初抓取的不完整版本（如 202608 仅 8 家券商）永久停留在页面；同时将 broker_recommend 刷新接入盘后任务列表
...
- [改进] 港股页今日 BOLL 报警按轨道区分排序，中轨报警优先展示，同轨内保持最新在前
- [改进] 港股页日内分钟连续回撤与涨幅榜以最新分钟为锚点，仅在向前 30 分钟窗口内重新计算并排序，避免 13/14 点等历史结果继续占据当前榜单
- [新功能] 港股页在日内分钟连续最大回撤 Top 5 下新增分钟涨幅 Top 5，按 `HK_LIST` 范围展示最近一组连续分钟快照的涨幅排名
- [改进] 港股页日内分钟连续最大回撤 Top 5 支持使用 `HK_LIST` 限定排行个股范围，不影响港股通成份主表和分钟数据采集
- [修复] 港股通分钟行情轮询改用腾讯批量实时行情，避免 Tushare `rt_hk_k` 权限不足导致分钟数据无法落库及日内连续最大回撤榜为空
...
- [改进] 港股监控右侧最近结束的最高连续回撤仅展示最近 5 个交易日内结束的数据
...
- [修复] 金股页当天涨幅会刷新超过 30 秒的陈旧行情快照，再按最新价与昨收计算；刷新失败不再把盘中旧快照冒充当日最终涨幅
- [修复] 首页打开个股分析报告时板块信号映射类型错误导致页面崩溃
- [修复] 港股通监控刷新按钮会强制同步最新成份快照后再加载主表，避免页面继续读取旧成份列表
- [修复] 港股通成份同步按上交所/深交所当前港股通标的名单过滤 AkShare/EastMoney 行情结果，避免已剔除个股继续累加保留
- [修复] 港股通标的中文简称为空时回退英文简称，避免 `00522` 等标的名称显示为空
- [修复] 港股通列表接口对历史空名称快照增加股票索引兜底，避免前端仍显示 `--`
- [新功能] 港股页右侧新增「近3日止跌」Tab：统计近 3 个交易日内刚结束连续下跌（至少连续 2 个交易日收跌、连跌未延续至最新交易日）的个股，展示连跌天数、累计跌幅、止跌以来反弹幅度与汇总均值（平均连跌天数/平均跌幅/平均反弹），点击条目定位左侧列表
- [改进] ETF、港股通监控、新高页面右侧 BOLL 推荐面板高度改为跟随左侧表格真实渲染高度，展开 K 线详情后自动同步
- [修复] 港股通成份同步在 AkShare 东财 HTTPS 断连时改用 EastMoney HTTP fallback，避免成份快照停留旧日期导致页面少股票
- [修复] 港股通监控展开个股 K 线后用最新日线回填左侧表格行情与 BOLL 距离，避免详情已有今日数据但主表仍显示 `--`
- [新功能] 港股通监控左侧表格新增现价距 BOLL 上/中/下轨距离列，并在列表接口返回对应三轨与距离字段
- [修复] 港股通监控左侧表格涨跌幅统一按最新交易日展示，避免个股日线滞后时混入旧交易日涨跌幅
- [修复] 港股通日 K 自动回填仅以 00700 单只判定导致部分成份股停留在上一交易日（如 6/24 数据缺失）；改为以成份股集合最小最新交易日判定，落后新浪最新日时全量回填，且按目标交易日幂等
- [新功能] 港股通监控：成份快照（AkShare）、盘中腾讯批量实时行情轮询落库、历史分钟回填脚本、Web `/hk-monitor` 页（30s 刷新）
...
- [新功能] 新高页：2026 至今全 A 股后复权收盘创新高统计，展开展示创新高日期倒序列表与 BOLL K 线
- [改进] 金股页个股展开「近 6 个月行情」K 线默认叠加 BOLL 布林带展示
...
- [新功能] 金股页当前月「全部金股明细」标题旁展示上月推荐个股在当月累计收益 Top3（后复权，每日更新）；新增 `/api/v1/broker-recommend/prev-month-current-top`。
- [修复] 金股页「全部金股明细」表格排序改为默认→升序→降序三态循环（修复累计收益列强制降序导致只能切换两次）
- [修复] 金股累计收益：截止日无当日复权因子时回退至上一交易日，避免除权日因子未同步导致假回撤
- [新功能] 金股页查看历史月份时，「全部金股明细」新增当前自然月累计收益列（如选 5 月金股可对照 6 月涨跌）；新增 `/api/v1/broker-recommend/current-month-returns`。
- [改进] 金股页「月累计收益」列标题与区间标注改为所选月份（如 `2025-05 · 05-06~05-30`），当前月亦展示；避免「当月」被误解为今天所在月。
- [修复] 金股「当月累计收益」误用后复权月初价对比不复权 `end_price`，除权月出现 -99% 等虚假回撤；改为与 `daily_returns` 后复权累计一致。
- [修复] 金股回测 API 未返回 `month_cumulative_return`，导致历史月份「当月累计收益」列全为 `--`。
- [改进] 金股页历史月份「全部金股明细」在累计收益后新增「当月累计收益」列（月初首个交易日至月末收盘价，后复权口径）。
- [改进] 金股策略回测卖出改为九转上升 8 当日收盘价平仓，下一交易日开盘买入符合条件的个股（不再升转降 T+1 开盘卖）。
- [改进] 金股策略回测买入信号由九转上升 1 调整为上升 5（入池与轮换补仓口径一致）。
- [改进] 金股策略回测改为每日检测上升 5、次日开盘买入（有空仓即买，不再要求先平仓）；移除历史胜率买入门槛；缓存 v11。
- [改进] 金股策略回测改为九转上升 + 近3日涨跌形态（--+ / +-+ 收盘买，+-- / -+- 收盘卖）；缓存 v12。
- [改进] 金股策略展开表买入日/卖出日悬停展示买卖理由（近三日走势、形态、九转信号）；缓存 v13。
- [改进] 金股策略回测买入恢复历史胜率过滤（须&gt;50%，无记录不买）；缓存 v14。
- [改进] 金股策略回测形态计算从买入日（持仓期首个交易日）起算，买入日之前交易日不参与；缓存 v22。
- [修复] 金股策略回测交易记录：无行情平仓不再生成空卖单，顺延失败回退最后收盘价真实平仓，持仓收益按固定买卖额计算；缓存 v30。
- [修复] 金股策略回测：现金不足固定买入额时用剩余现金继续开仓，避免上月小幅亏损后后续月份无交易；九转预取优先 SQLite 且禁用逐条回退以防接口限流；缓存 v31。
- [改进] 金股策略回测：投入本金调整为 20、每笔固定 1 单位开仓，总收益按结算总资产相对投入本金计算；缓存 v32。
- [改进] 金股策略回测：投入本金按实际开仓累计（每笔 +1，非固定初始资金池），总收益=结算总资产/累计投入本金-1；缓存 v33。
- [改进] 金股策略回测：开仓时九转 up_count 超过 11 不买入（允许 1..11）；缓存 v34。
- [改进] 金股策略回测：九转买入 up_count 上限由 11 调整为 10（允许 1..10）；缓存 v35。
- [改进] 金股策略回测重写：总资金固定；升转降信号日 N 股 T+1 开盘均摊买、T+2 开盘卖；无信号日 T+1 清仓后暂停；移除形态/胜率/持仓等旧限制；缓存 v36。
- [改进] 金股策略回测：交易仅限当月，末交易日升转降忽略且 T+2 超出当月不开仓；缓存 v37。
- [改进] 金股策略回测：升转降买入仅接受前日上升 2..11，升 1 与升 12+ 转降忽略；缓存 v38。
- [改进] 金股策略回测：升转降买入前日上升上限由 11 调整为 10（允许 2..10）；缓存 v39。
- [新功能] 金股策略 Tab 新增升转降分档统计表（升 2..10 各档交易次数、平均收益、胜率）；缓存 v40。
- [改进] 金股策略回测买入仅接受升 2/7/8 转降；统计表同步为三档；缓存 v41。
- [改进] 金股策略回测买入收窄为仅升 8 转降；缓存 v42。
- [改进] 金股策略回测买入改为升 1..8 转降；统计表同步为八档；缓存 v43。
- [改进] 金股策略回测卖出改为 T+2 开盘亏损卖、盈利延至 T+3 收盘卖（T+3 超出当月则月末收盘卖）；缓存 v44。
- [改进] 金股策略回测盈利延卖：T+3 超出当月改为下月交易日收盘价卖出；缓存 v45。
- [修复] 金股策略回测收益虚高：移除买入时补足固定本金的注入逻辑，有持仓时不再叠加开新仓；缓存 v46。
- [改进] 金股升转降盈利卖出：T+2 盈利后 T+3 收盘超买入日收盘价则继续持有，直至收盘低于买入开盘价再卖出；缓存 v47。
- [改进] 金股升转降月末强制收盘清仓，盈利跟踪不再跨月延续；无行情顺延开盘清仓；缓存 v48。
- [新功能] 月度金股页券商组合收益曲线下新增升转降个股列表（`/{month}/up-to-down-daily`），支持按信号日筛选（所选日无信号则空表）。
- [改进] 升转降个股表支持 Todo 式划线标记（可取消），状态本地持久化。
- [改进] 升转降个股列表新增降 1..8 升信号展示（与升转降同表、按信号日筛选）。
- [改进] 九转反转个股改为左右双列表：左侧升转降、右侧降转升，共用信号日筛选。
- [改进] 九转反转与升转降策略回测：月初第 1 个交易日与上月末对比九转（跨月连续序列）；策略缓存 v49。
- [修复] 九转反转列表误将当月最新交易日当作月末忽略，导致进行月中当日升转降/降转升不展示。
- [改进] 金股策略回测改为每笔固定金额买入，总收益按平仓后总资产相对初始本金计算；缓存 v29。
- [改进] 金股策略回测恢复近 3 日形态买卖，取消最多 3 只买入/持仓限制（符合条件且胜率&gt;50% 均可等权买入）；缓存 v28。
- [改进] 金股策略回测暂时关闭近 3 日形态买卖，仅保留九转上升买入与升转降 T+1 确认卖出；缓存 v27。
- [改进] 金股策略回测买入排序：历史 max_return==max_drawdown 且均为正时优先级最高，其余仍按绝对值差降序；缓存 v26。
- [改进] 金股策略回测买入排序：历史最高/最低持仓收益绝对值相同时优先级最高，其余仍按绝对值差降序；缓存 v25。
- [修复] 金股策略回测形态卖出改为以该笔持仓买入日为锚点重算近 3 日形态（买入后未满 3 个交易日不触发形态卖）；缓存 v24。
- [改进] 金股策略回测买入优先级改为当月个股历史推荐统计最高/最低持仓收益绝对值差降序（与展开历史统计一致），历史胜率&gt;50% 作为排序后过滤；缓存 v23。
- [改进] 金股策略回测买入优先级改为盈利预测最高/最低绝对值差降序，历史胜率&gt;50% 作为排序后过滤；缓存 v21。
- [改进] 金股策略回测升转降卖出改为 T+1 仍下降则收盘卖出（取消次日开盘卖）；缓存 v20。
- [改进] 金股策略回测月末顺延清仓上限由 10 个交易日提高至 20 个；缓存 v19。
- [改进] 金股策略回测月末顺延清仓：月末无行情时按交易日依次顺延至有开盘价/收盘价之日开盘卖出（最多 10 日、可跨月），并预取顺延区间 OHLC；缓存 v18。
- [修复] 金股策略回测卖出价缺失：升转降开盘卖缺开盘价时回退收盘价，月末清仓走真实平仓，策略月末日 K 线从 OHLC 补齐；缓存 v16。
- [改进] 金股策略回测卖出新增九转升转降次日开盘卖，与形态收盘卖并存；缓存 v15。
- [修复] 金股策略回测买入/卖出价：兼容历史日序列中已后复权 open 与不复权收盘混存，避免开盘价重复乘复权因子；展开行展示不复权成交价。
- [改进] 金股策略回测月度展开列表按买入日升序展示持仓腿。
- [改进] 金股策略回测 Tab 支持月份区间选择器与「重新计算」，可自定义 `start_month`/`end_month` 回测范围。
- [改进] 金股策略回测：买入范围限定为目标价绝对值差 Top3，最多持仓 3 只，仅平仓后可补仓（不再每日等权全仓调仓）。
- [改进] 金股策略回测买入前过滤历史推荐胜率：无历史记录或低于 55% 均不买入；展开行展示 `hist_win_rate`。
- [改进] 金股策略回测支持 `start_month`/`end_month` 区间过滤；前端默认统计今年 1 月至今。
- [改进] 金股策略回测月末最后一个交易日收盘强制清仓，末日本地不再开新仓。
- [改进] 金股策略回测展开行新增买入价、卖出价（后复权成交价，与持仓收益计算口径一致）。
- [修复] 金股策略回测展开行按持仓腿记录买卖日；放宽九转升转降判定（↓计数/下跌九转/上升归零），月中反转不再误显示月末卖出。
- [改进] 金股策略回测改为第 1 日九转上升 1 入池、第 2 日开盘买入；上升转下降时 T+1 开盘卖出并买入前一日上升 1 个股，等权调仓。
- [修复] 金股策略回测首次计算超时：改为后台异步计算 + 单次交易日历与按日批量预取九转，前端请求超时延长至 120s。
- [修复] 金股策略回测实现被合并截断且九转过滤缺少 ORM 导入，导致策略 Tab 长期无数据。
...
- [新功能] 快测页多持仓（top_n>1）时在收益贡献 Top5 上方新增「选股顺位收益贡献」表，按 Top1~TopN 汇总已平仓盈亏；交易记录新增 pick_rank 字段。
- [新功能] LGB 页回测模拟多持仓（top_n>1）时在交易明细下方新增「选股顺位收益贡献」表，按 Top1~TopN 汇总各顺位盈亏与胜率。
- [新功能] 寻股页盘中/盘后回测多持仓时在交易记录下方新增「选股顺位收益贡献」表，按 Top1~TopN 汇总各顺位盈亏与胜率；交易记录新增 pick_rank 字段。
- [改进] 寻股页回测顺位收益表固定展示 Top1~Top4，并移至交易记录表格上方。
- [新功能] 因子回测页多持仓（top_n>1）时在交易明细下方新增「选股顺位收益贡献」表，按 Top1~TopN 汇总已平仓顺位盈亏。
- [修复] 金股页「历史统计」批量接口逐月 OHLC 重算导致 30s 超时、表格长期显示 `--`；改为基于持仓期序列+复权因子快速聚合，并延长前端请求超时。
- [修复] 金股页持仓期收益/历史统计「最低」在除权月误用未复权收盘价重算，导致如兖矿能源出现 -36% 等虚假回撤；K 线仍展示不复权 OHLC，累计收益改用后复权收盘价。
- [修复] 金股页历史统计「最低」收益与展开历史不一致：统计改为与展开 K 线相同的 OHLC 同步口径，并以历次持仓期末收益的最小值展示（不再用路径峰值回撤）。
- [改进] 金股页「累计收益」列：表头仅保留标题，持仓区间显示在每行收益率下方。
- [新功能] 金股页「全部金股明细」新增「历史统计」列：展示历次有回测记录的推荐持仓期胜率、最高收益与最大回撤；新增 `/api/v1/broker-recommend/historical-recommend-stats` 接口。
- [改进] 金股页当前月表格红色高亮历史推荐月份数最多的 10 只股票（含推荐次数角标）。
- [修复] 金股页 K 线 OHLC 与收盘价口径不一致（复权 OHLC + 未复权收盘价）：统一从未复权 OHLC 同步 open/high/low/close，并重算持仓期收益。
- [修复] 金股页个股展开历史 K 线：默认仅展示所选推荐月持仓期 K 线（与表格累计收益区间一致），过滤无收盘价占位行，移除 overlay 覆盖；可选查看全部推荐月。
- [修复] 金股页切换月份时月末价/九转/盈利预测/累计收益与所选月份不一致：请求竞态防护 + 仅采用 month 字段匹配的 backtest/enrichment 数据，列标题标注截至日与回测区间。
- [新功能] 金股页「全部金股明细」展开个股可查看历次推荐月份持仓期 K 线；新增 `/api/v1/broker-recommend/stock/{ts_code}/history` 接口。
...
- [改进] 评级变化报告与摘要：`买入`→`持有` 单独使用 `🟡`；其他升高/下降仍为 `✅`/`❌`；列表顺序为升高、买入转持有、一般下降；规则集中在 `src/utils/rating_trend.py` 并与 `main` 摘要保持一致
