...
@pytest.mark.parametrize("message", ["check risk", "follow up"])
def test_chat_stock_resolution_ignores_lowercase_english_prose(message: str) -> None:
    with patch(
        "src.services.name_to_code_resolver.resolve_name_to_code",
        side_effect=lambda token: token.upper(),
    ):
        assert agent_endpoint._try_resolve_stock_code_from_message(message) is None
