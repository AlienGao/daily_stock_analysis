...
class TestSectorFactor:
    ...
    def test_limit_times_scoring(self, factor):
        ...
        assert scores["B.SZ"] == 40.0
        assert scores["C.BJ"] == 60.0
        assert scores["D.SH"] == 80.0
        assert scores["E.SZ"] == 100.0
    ...
    def test_pct_chg_fallback(self, factor):
        ...
        scores = factor.score(df)
        assert scores["A.SH"] == 47.5
        assert scores["B.SZ"] == 50.0
    ...
    def test_describe_limit_times(self, factor):
        ...
        assert "连板龙头" in reasons["A.SH"][0]
        assert "首板涨停" in reasons["B.SZ"][0]
        assert "2连板" in reasons["C.BJ"][0]
