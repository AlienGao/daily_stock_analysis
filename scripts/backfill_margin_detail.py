#!/usr/bin/env python3
"""回填 margin_detail 历史融资融券数据。

从 Tushare margin_detail API 逐日拉取全市场数据并写入本地 DB。
支持断点续跑：已缓存的日期自动跳过。

用法:
    python scripts/backfill_margin_detail.py              # 回填全部缺失日期
    python scripts/backfill_margin_detail.py --days 60    # 仅最近 60 个交易日
    python scripts/backfill_margin_detail.py --dry-run    # 预览待回填日期
"""
...
def get_missing_dates(db, limit_days: int = 0) -> list:
    """返回 stock_daily 中有数据但 margin_detail 中缺失的日期列表。"""
    ...
    with db.get_session() as s:
        cached = {
            ...
            s.execute(text("SELECT DISTINCT trade_date FROM margin_detail")).fetchall()
        }
        all_dates = [
            _to_date(row[0]) for row in
            s.execute(
...
def main():
    parser = argparse.ArgumentParser(description="回填 margin_detail 历史数据")
    ...
    for i, d_obj in enumerate(missing):
        ...
        try:
            df = tf.get_bulk_margin_detail(trade_date)
